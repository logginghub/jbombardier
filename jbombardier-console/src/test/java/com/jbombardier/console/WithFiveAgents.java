...
public class WithFiveAgents {
    ...
    public static void main(String[] args) {
        ...
        builder().addTest(test(SleepTest.class).name("delay-20").properties("delay=20").threads(10))
                 ...
                 .addAgent(agent().address("localhost").port(20003).name("agent3"))
                 .addAgent(agent().address("localhost").port(20004).name("agent4"))
                 .addAgent(agent().address("localhost").port(20005).name("agent5"))                 
                 .autostart(5)
                 .execute();
