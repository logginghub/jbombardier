...
/**
 * A blatant copy of the JUnit Java task, but with some bits hard-wired to run
 * the jbombardierHeadless main method.
 * 
 * @author James
 * 
 */
public class JBombardierAntTask extends Task {
    ...
    // Stuff pinched from the junit java task :
     ...
     * Normal constructor
     */
    public JBombardierAntTask() {
        getCommandLine().setClassname("JBombardierHeadless");
        setFork(true);
    }
    ...
    public void execute() throws BuildException {
        log("Running JBombardierAnt task...", Project.MSG_INFO);
        
        File savedDir = dir;
        Permissions savedPermissions = perm;

        cmdl.createArgument().setValue(configPath);
        cmdl.createArgument().setValue(Long.toString(warmupTime));
        cmdl.createArgument().setValue(Long.toString(sampleTime));
        cmdl.createArgument().setValue(Integer.toString(agentsRequired));
        cmdl.createArgument().setValue(Long.toString(timeToWaitForAgents));
        cmdl.createArgument().setValue(reportsPath);
        ...
        try {
            ...
            log("Error code was " + err, Project.MSG_INFO);
     ...
     *         a separate VM (fork = "yes") or a security manager was installed
     *         that prohibits ExitVM (default).
     * 
     * @throws BuildException
    ...
    /**
     * Deprecated: use nested arg instead. Set the command line arguments for
     * the class.
     * 
     * @param s
     *            arguments.
    ...
    /**
     * Set the name of the property in which the return code of the command
    ...
    /**
     * If true, then fail if the command exits with a returncode other than
     * zero.
     * 
     * @param fail
     *            if true fail the build when the command exits with a nonzero
     *            returncode.
     */
    ...
    /**
     * Set whether error output of exec is logged. This is only useful when
     * output is being redirected and error output is desired in the Ant log.
     * 
     * @param logError
     *            get in the ant log the messages coming from stderr in the case
     *            that fork = true.
     */
    ...
    /**
     * Set the property name whose value should be set to the error of the
     * process.
     * 
     * @param errorProperty
     *            property name.
    ...
    /**
     * Executes the given classname with the given arguments as it were a
    ...
    /**
     * On VMS platform, we need to create a special java options file containing
     * the arguments and classpath for the java command. The special file is
     * supported by the "-V" switch on the VMS JVM.
     * 
     * @param exe
     *            executable.
     * @param command
    ...
    /**
     * Executes the given classname with the given arguments as if it were a
     * command line application.
     * 
     * @param classname
